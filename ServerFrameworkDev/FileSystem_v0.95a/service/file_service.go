...
// 在现有代码基础上增加以下方法
// 获取文件元数据
func (f *FileService) GetFileMeta(fileID uint64) (*model.File, error) {
	var file model.File
	if err := global.DB.Preload("Permissions").First(&file, fileID).Error; err != nil {
		return nil, err
	}
	return &file, nil
}
...
// 删除文件（需要事务操作）
// 在FileService结构体下添加
func (f *FileService) CheckFileOwner(userID, fileID uint64) bool {
	var file model.File
	if err := global.DB.Select("uploader_id").First(&file, fileID).Error; err != nil {
		return false
	}
	return file.UploaderID == userID
}

func (f *FileService) DeleteFile(fileID uint64) error {
	return global.DB.Transaction(func(tx *gorm.DB) error {
		// 先删除权限记录
		if err := tx.Where("file_id = ?", fileID).Delete(&model.FilePermission{}).Error; err != nil {
			return err
		}
